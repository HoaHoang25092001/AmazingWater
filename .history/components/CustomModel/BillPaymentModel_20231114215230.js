...
const BillPaymentModel = ({ visible, onClose }) => {
    const [readingRouteId, setReadingRouteId] = useState("");
    ...
    useEffect(() => {
        if (invoice) {
            ...
            setReadingRouteField(invoice.tuyenDoc);
            setReadingRouteId(invoice.chiSoDongHo.tuyenId);
        }
    }, [invoice]);
                        ...
                        <Box w="90%" maxW="400">
                            <VStack space="md">
                                ...
                                <Box maxW="100%" alignItems="center">
                                    <Input mx="2" placeholder="Tuyến đọc" w="100%" value={readingRouteField} isReadOnly/>
                                </Box>
                                <Text fontFamily="Quicksand_500Medium">Khách hàng:</Text>
                                <Box maxW="100%" alignItems="center">
                                    <Input mx="2" placeholder="Tên khách hàng" w="100%" value={customerNameField} isReadOnly/>
                                </Box>
