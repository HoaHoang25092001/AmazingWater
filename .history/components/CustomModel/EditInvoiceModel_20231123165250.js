...
const EditInvoiceModel = ({ visible, onClose }) => {
  ...
  `;
    ...
    useEffect(() => {
        if (invoice) {
            setCustomerNameField(invoice.tenKH);
            setCustomerIdField(invoice.khachHangKeyId);
            setCustomerTaxCodeField(invoice.maSoThue);
            setCustomerPhoneNumberField(invoice.dienThoai);
            setCustomerAddressField(invoice.diaChi);
        }
    }, [invoice]);
